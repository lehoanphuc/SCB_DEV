...
public abstract class WidgetBase : UserControl
{
    ...
    public void RedirectToActionPage(string action, string paraUrl, bool endResponse = true)
    {
        ...
        Response.Redirect(SmartPortal.Common.Encrypt.EncryptURL(link), endResponse);
    ...
    public bool IBCheckPermitPageAction(string action)
    {
        string pageid = SmartPortal.Common.Encrypt.GetURLParam(System.Web.HttpContext.Current.Request.RawUrl)["p"].ToString();
        return PagesBLL.IBCheckPermitPageAction(pageid, Session["userID"].ToString(), action);
    }
}
