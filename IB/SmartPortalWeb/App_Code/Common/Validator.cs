...
namespace SmartPortal.Common
{
    ...
    {
        /// <summary>
        /// Check phone number only number not include space.
        /// </summary>
        public static bool IsPhoneNumber(string strToCheck)
        {
            bool rs = true;
            rs = Regex.IsMatch(strToCheck, "^[0-9]+$");
            if(strToCheck.Length <6 || strToCheck.Length>15)
            {
                rs = false;
            }
        ...
        public static bool CheckYearOld( string date,int old=18)
        {
            return (DateTime.Now < SmartPortal.Common.Utilities.Utility.IsDateTime1(date.Trim()).AddYears(old));
        }
    }
}
