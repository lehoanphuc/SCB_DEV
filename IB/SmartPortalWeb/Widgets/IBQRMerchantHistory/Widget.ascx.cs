...
public partial class Widgets_IBTransactionHistory1_Widget : WidgetBase
{
    ...
    private void ShowFD(Hashtable hashtable)
    {
        try
        {
            if (hashtable != null)
            {
                ...
                lblCurrency_DD.Text = hashtable["CURRENCYID"].ToString();
    ...
    private void ShowDetailDD(string acctno)
    {
        try
        {
            ...
            if (txtFromDate.Text != "" && txtToDate.Text != "")
            {
                ...
                {
                    case "WL":
                        ds = acct.getQRTransactionHisWalletAcct(Session["userID"].ToString(), "WL_LOADTRANHIS", acctno, txtFromDate.Text, txtToDate.Text, lastID, ref ErrorCode, ref ErrorDesc);
                        break;
                    default:
                        ds = acct.getQRTransactionHis(Session["userID"].ToString(), "MB000105", acctno, txtFromDate.Text, txtToDate.Text, lastID, ref ErrorCode, ref ErrorDesc);
                        break;
                }
            }
            else
            {
                ...
                {
                    case "WL":
                        ds = acct.getQRTransactionHisWalletAcct(Session["userID"].ToString(), "WL_LOADTRANHIS", acctno, DateTime.Now.AddDays(-7).ToString("dd/MM/yyyy"), DateTime.Now.ToString("dd/MM/yyyy"), lastID, ref ErrorCode, ref ErrorDesc);
                        break;
                    default:
                        ds = acct.getQRTransactionHis(Session["userID"].ToString(), "MB000105", acctno, DateTime.Now.AddDays(-7).ToString("dd/MM/yyyy"), DateTime.Now.ToString("dd/MM/yyyy"), lastID, ref ErrorCode, ref ErrorDesc);
                        break;
                }
            }
            ...
            sT.Append("<th align='center' width='10%'>");
            sT.Append(Resources.labels.date);
            sT.Append("</th>");

            sT.Append("<th align='center' width='10%'>");
            sT.Append(Resources.labels.time);
            sT.Append("</th>");

            sT.Append("<th align='center' width='10%'>");
            sT.Append(Resources.labels.transactionno);
            sT.Append("</th>");
            ...
            sT.Append("<th align='center' width='10%'>");
            sT.Append(Resources.labels.userid);
            sT.Append("</th>");

            sT.Append("<th align='center' width='10%'>");
            sT.Append(Resources.labels.PhoneNumber);
            sT.Append("</th>");

            sT.Append("<th align='center' width='20%'>");
            sT.Append(Resources.labels.amount);
            sT.Append("</th>");

            sT.Append("<th align='center' width='20%'>");
            sT.Append(Resources.labels.Balance);
            sT.Append("</th>");
            ...
            if (ds != null && ds.Tables.Count > 0)
            {
                ...
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    ...
                    newRow["Transaction date"] = Utility.FormatDatetime(row["TRANDATE"].ToString(), "dd/MM/yyyy", DateTimeStyle.Date);
                    newRow["Transaction time"] = row["TRANTIME"].ToString();
                    newRow["Transaction No"] = row["IPCTRANSID"].ToString();
                    newRow["QR Name"] = row["RECEIVERNAME"].ToString();
                    newRow["User ID"] = row["USERID"].ToString();
                    newRow["Phone Number"] = row["SENDERPHONE"].ToString();
                    ...
                    newRow["Balance"] = Utility.FormatMoney(Utility.FormatStringCore(row["ENDBALANCE"].ToString()), row["CCYID"].ToString().Trim());
                    ...
                    sT.Append("<td>");
                    sT.Append(Utility.FormatDatetime(row["TRANDATE"].ToString(), "dd/MM/yyyy", DateTimeStyle.Date));
                    sT.Append("</td>");

                    sT.Append("<td>");
                    sT.Append(row["TRANTIME"].ToString());
                    sT.Append("</td>");


                    sT.Append("<td>");
                    sT.Append(Utility.FormatStringCore(row["IPCTRANSID"].ToString()));
                    sT.Append("</td>");

                    sT.Append("<td>");
                    sT.Append(Utility.FormatStringCore(row["RECEIVERNAME"].ToString()));
                    sT.Append("</td>");

                    sT.Append("<td>");
                    sT.Append(Utility.FormatStringCore(row["USERID"].ToString()));
                    sT.Append("</td>");

                    sT.Append("<td>");
                    sT.Append(Utility.FormatStringCore(row["SENDERPHONE"].ToString()));
                    sT.Append("</td>");
                    ...
                    sT.Append("<td style='text-align:right'>");
                    sT.Append(Utility.FormatMoney(Utility.FormatStringCore(row["ENDBALANCE"].ToString()), row["CCYID"].ToString().Trim()) + " " + row["CCYID"].ToString().Trim());
    ...
    public DataTable CreateTableExport()
    {
        ...
        DataColumn colTranTime = new DataColumn("Transaction time");
        DataColumn colTranNo = new DataColumn("Transaction No");
        DataColumn colQRName = new DataColumn("QR Name");
        DataColumn colUserID = new DataColumn("User ID");
        DataColumn colPhoneNumber = new DataColumn("Phone Number");
        DataColumn colCarddit = new DataColumn("Amount");
        ...
        tblExport.Columns.Add(colTranTime);
        tblExport.Columns.Add(colTranNo);
        tblExport.Columns.Add(colQRName);
        tblExport.Columns.Add(colUserID);
        tblExport.Columns.Add(colPhoneNumber);
        tblExport.Columns.Add(colCarddit);
        tblExport.Columns.Add(colBalance);
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            dt = TBLDATA;
            ExportExcel(TBLDATA, "QR_Merchant_Report", Resources.labels.chitietgiaodichtaikhoanthanhtoan);
        }
        catch (Exception ex)
        {
