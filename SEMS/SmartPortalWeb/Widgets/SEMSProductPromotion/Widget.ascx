<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Widget.ascx.cs" Inherits="Widgets_SEMSProductPromotionManager_Widget" %>
    ...
    <ContentTemplate>
        <div class="subheader">
            <h1 class="subheader-title">
                <%=Resources.labels.productpromotion %>
            </h1>
        </div>
        <div class="loading">
        ...
        <div id="divToolbar">
            ...
            <asp:Button ID="btnInactive" CssClass="btn btn-primary" runat="server" Text="<%$ Resources:labels, InAndActive %>" OnClick="btnInactive_OnClick" OnClientClick="Loading(); return ConfirmInactive1();" />
        </div>
        <div id="divResult" runat="server" class="divrs">
            ...
            <asp:GridView ID="gvProductDiscount" CssClass="table table-hover" runat="server" BackColor="White" BorderColor="#CCCCCC" BorderStyle="Solid" BorderWidth="1px" CellPadding="3"
                Width="100%" AutoGenerateColumns="False" OnRowDataBound="gvProductDiscount_RowDataBound" PageSize="15" OnRowCommand="gvProductDiscount_RowCommand" OnRowDeleting="gvProductDiscount_RowDeleting">
                <Columns>
                    <asp:TemplateField>
                        <ItemTemplate>
                    ...
                    <asp:TemplateField HeaderText="<%$ Resources:labels, promotionname %>">
                        <ItemTemplate>
                    ...
                    <%--<asp:TemplateField HeaderText="<%$ Resources:labels, promotiontype %>">
                        <ItemTemplate>
                            <asp:Label ID="lblPromotionType" runat="server"></asp:Label>
                        </ItemTemplate>
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:TemplateField>--%>
                    ...
                    <asp:TemplateField HeaderText="<%$ Resources:labels, edit %> ">
                        <ItemTemplate>
                            <asp:LinkButton ID="lbEdit" runat="server" CssClass="btn btn-primary" CommandName='<%#IPC.ACTIONPAGE.EDIT %>' CommandArgument='<%#Eval("PromotionID") %>' OnClientClick="Loading();">Edit</asp:LinkButton>
                        </ItemTemplate>
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="<%$ Resources:labels, delete %>">
                        <ItemTemplate>
                            <asp:LinkButton ID="lbDelete" runat="server" CssClass="btn btn-secondary" CommandName='<%#IPC.ACTIONPAGE.DELETE %>' CommandArgument='<%#Eval("PromotionID") %>' OnClientClick="Loading(); return Confirm();">Delete</asp:LinkButton>
                        </ItemTemplate>
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:TemplateField>

                    <asp:TemplateField HeaderText="<%$ Resources:labels, clone %>">
                        <ItemTemplate>
                            <asp:LinkButton ID="lblClone" runat="server" CssClass="btn btn-secondary" CommandName="CLONE" CommandArgument='<%#Eval("PromotionID") %>' OnClientClick="Loading();">Clone</asp:LinkButton>
                        </ItemTemplate>
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:TemplateField>
                </Columns>
...
<script type="text/javascript">
    ...
    function ConfirmInactive1() {
        debugger;
        var hdf = document.getElementById("<%= hdCounter.ClientID %>");
        if (hdf.value == 0) {
            alert('<%=Resources.labels.pleaseselectbeforeInactive %>');
            return false;
        } else {
            return ConfirmInactive('<%=Resources.labels.banchacchanmuoninactive %>');
        }
    }

    function ConfirmInactive() {
        return confirm('<%=Resources.labels.banchacchanmuoninactive %>');
    }
</script>
