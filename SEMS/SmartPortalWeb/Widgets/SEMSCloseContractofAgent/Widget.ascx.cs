...
public partial class Widgets_SEMSCloseContractofAgent_Widget : WidgetBase
{
   ...
   protected void BindData()
    ...
    protected void btnAccept_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            setPara(info);
            ds = _service.CallStore("SEMS_BO_CLOSE_WALAGT", info, "Close contract of Agent", "N", ref IPCERRORCODE, ref IPCERRORDESC);
            if (IPCERRORCODE == "0")
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
