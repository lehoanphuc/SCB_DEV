        ...
        try
        {
            if (!IsPostBack)
            {
                if (Request["uid"] != null)
                {
                    ...
                    gvRoles.DataSource = RB.LoadForUser(Request["uid"].ToString(),"SEMS");
                    gvRoles.DataBind();
                    
                    //an hien nut save
                    if (Request["t"] != null)
                    {
                        if (Request["t"].ToString() == "t")
                        {
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (Request["uid"] != null)
        {
            ...
            foreach (GridViewRow gvr in gvRoles.Rows)
            {
                ...
                {
                    UIR.Insert(Utility.IsInt(lblRoleID.Text), Request["uid"].ToString(), true);
                }
                else
                {
                    UIR.Insert(Utility.IsInt(lblRoleID.Text), Request["uid"].ToString(), false);
                }

                 //Write Log
                SmartPortal.Common.Log.WriteLogDatabase(System.Configuration.ConfigurationManager.AppSettings["USERINROLE"], System.Configuration.ConfigurationManager.AppSettings["INSERT"], Request.Url.ToString(), Session["userName"].ToString(), Request.UserHostAddress, System.Configuration.ConfigurationManager.AppSettings["TBLUSERINROLE"], System.Configuration.ConfigurationManager.AppSettings["USERNAME"], "", Request["uid"].ToString());
