...
<table class="style1">
    ...
    style="width: 16px; height: 15px" /><asp:HyperLink ID="lblTitle" runat="server" 
                        Text='<%# Eval("Title") %>' NavigateUrl='<%#SmartPortal.Common.Encrypt.EncryptURL(System.Configuration.ConfigurationManager.AppSettings["newsdetail"]+"&nid="+Eval("NewsID").ToString()) %>'></asp:HyperLink>
