...
public partial class Widgets_SEMSCURRENCYFX_Widget : WidgetBase
{
    ...
    private void LoadDll()
    {
        try
        {
            ...
            ddlFromCCYID.Items.Insert(0, new ListItem(Resources.labels.tatca, "ALL"));

            ddlToCCYID.DataSource = data;
            ddlToCCYID.DataTextField = "CCYID";
            ddlToCCYID.DataValueField = "CCYID";
            ddlToCCYID.DataBind();
            ddlToCCYID.Items.Insert(0, new ListItem(Resources.labels.tatca, "ALL"));
        }
        catch (Exception ex)
        {
            SmartPortal.Common.Log.RaiseError(System.Configuration.ConfigurationManager.AppSettings["sysec"],
    ...
    private void BindData()
    {
        try
        {
            ...
            litError.Text = string.Empty;
            DataTable dtCurrency = new Currency().SearchCurrencyFx(Utility.KillSqlInjection(ddlFromCCYID.SelectedValue.Trim()), Utility.KillSqlInjection(ddlToCCYID.SelectedValue.Trim()), pageSize, recordIndex,  ref IPCERRORCODE, ref IPCERRORDESC).Tables[0];
            if (IPCERRORCODE == "0")
            {
    ...
    protected void gvCurrencyList_OnRowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            CheckBox cbxSelect;
            LinkButton lbFromCCYID, lbToCCYID, lbEdit, lbDelete;
            Label lblDesc, lblStatus;
            DataRowView drv;
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                lbFromCCYID = (LinkButton) e.Row.FindControl("lbFromCCYID");
                lbToCCYID = (LinkButton) e.Row.FindControl("lbToCCYID");
                ...
                lbFromCCYID.Text = drv["FROMCCYID"].ToString();
                lbToCCYID.Text = drv["TOCCYID"].ToString();
