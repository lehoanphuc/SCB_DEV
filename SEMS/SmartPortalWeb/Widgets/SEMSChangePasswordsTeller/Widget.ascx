...
<script>
    function validate() {
        ...
        if (document.getElementById('<%=txtNewPassword.ClientID%>').value == document.getElementById('<%=txtOldPassword.ClientID%>').value) {
            alert('<%=Resources.labels.newpasswordnotsameoldpassword %>');
            document.getElementById('<%=txtNewPassword.ClientID %>').focus();
            return false;
        }
        if (document.getElementById('<%=txtNewPassword.ClientID%>').value !=
