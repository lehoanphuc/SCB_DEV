    ...
    <ContentTemplate>
        <div class="subheader">
            <h1 class="subheader-title">
                <%=Resources.labels.setpushnotification %>
            </h1>
        </div>
        <div class="loading">
        ...
        <div id="divToolbar">
            <asp:Button ID="btnAdd" CssClass="btn btn-primary" runat="server" Text="<%$ Resources:labels, themmoi %>" OnClick="btnAdd_Click" OnClientClick="Loading();" />
        ...
        <div id="divResult" runat="server">
            ...
            <asp:GridView ID="gvSTV" CssClass="table table-hover" runat="server" AutoGenerateColumns="False"
                ...
                CellPadding="3" Width="100%" OnRowDataBound="gvSTV_RowDataBound" OnRowCommand="gvSTV_RowCommand" OnRowDeleting="gvSTV_RowDeleting">
                <Columns>
                    <asp:TemplateField>
                        <ItemTemplate>
                    ...
                    <asp:TemplateField HeaderText='<%$ Resources:labels, notificationname %>'>
                        <ItemTemplate>
                            <asp:LinkButton ID="lbPNName" runat="server" CommandName='<%#IPC.ACTIONPAGE.DETAILS %>' CommandArgument='<%#Eval("ID") %>' OnClientClick="Loading();"></asp:LinkButton>
                        </ItemTemplate>
                    </asp:TemplateField>
                    ...
                    <asp:TemplateField HeaderText="<%$ Resources:labels, edit %> ">
                        <ItemTemplate>
                            <asp:LinkButton ID="lbEdit" runat="server" CssClass="btn btn-primary" CommandName='<%#IPC.ACTIONPAGE.EDIT %>' CommandArgument='<%#Eval("ID") %>' OnClientClick="Loading();">Edit</asp:LinkButton>
                        </ItemTemplate>
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="<%$ Resources:labels, delete %>">
                        <ItemTemplate>
                            <asp:LinkButton ID="lbDelete" runat="server" CssClass="btn btn-secondary" CommandName='<%#IPC.ACTIONPAGE.DELETE %>' CommandArgument='<%#Eval("ID") %>' OnClientClick="Loading(); return Confirm();">Delete</asp:LinkButton>
                        </ItemTemplate>
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:TemplateField>
                </Columns>
...
<script type="text/javascript">
    function SelectCbx(obj) {
        ...
        if (obj.checked) {
            for (i = 0; i < count; i++) {
                if (elements[i].cells[0].children[0].type == 'checkbox' && elements[i].cells[0].children[0].id != 'ctl00_ctl20_gvSTV_ctl01_cbxSelectAll') {
