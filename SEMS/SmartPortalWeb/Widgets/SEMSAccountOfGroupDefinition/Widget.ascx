<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Widget.ascx.cs" Inherits="Widgets_SEMSAccountOfGroupDefinition_Widget" %>
...
<asp:UpdatePanel ID="UpdatePanel1" runat="server">
    <ContentTemplate>
                                ...
                                <div class="collapse in" id="AdvanceSearch">
                                    <div class="panel-content form-horizontal p-b-0" style="display: block; margin-left: 2%">
                                        ...
                                        <div class="row">
                                            <div class="col-sm-5">
                                                <div class="form-group">
                                                    <label class="col-sm-5 control-label"><%=Resources.labels.groupname %></label>
                                                    <div class="col-sm-7">
        ...
        <%-- Table View --%>
        <asp:Panel runat="server" Visible="false" ID="pnPanel">
                                ...
                                <table class="table table-bordered" style="border: 1px solid rgba(0, 0, 0, 0.1); border-style: Solid;">
                                    <thead style="background-color: #7A58BF; color: #FFF;">
                                        <tr>
                                            ...
                                            <th class="title-repeater"><%=Resources.labels.desc%></th>
                        ...
                        <ItemTemplate>
                            <tr>
                                <td class="title-repeater">
                                    <input name="item1" class="check" value="<%#Eval("GRP_ID") + "+" + Eval("AC_GRP_NAME") %>" onclick="ConfigCheckbox(this)" type="checkbox">
                                </td>
                                <td class="action">
                                    <asp:LinkButton runat="server" CommandArgument='<%#Eval("GRP_ID") + "+" + Eval("AC_GRP_NAME")%>' CommandName='<%#IPC.ACTIONPAGE.DETAILS%>' OnClientClick="Loading();"><%#Eval("GRP_ID") %></asp:LinkButton>
                                </td>
                                <td><%#Eval("AC_GRPDEF") %></td>
                                <td><%#Eval("ACNO")%></td>
                                <td><%#Eval("AC_GRP_NAME")%></td>
                                <td><%#Eval("BAC_GRP_NAME")%></td>
                                <td class="action" style="text-align: center">
                                    <asp:LinkButton runat="server" class="btn btn-primary" CommandArgument='<%#Eval("GRP_ID") + "+" + Eval("AC_GRP_NAME")%>' CommandName='<%#IPC.ACTIONPAGE.EDIT%>' OnClientClick="Loading();">Edit</asp:LinkButton>
                                </td>
                                <td class="action" style="text-align: center">
                                    <asp:LinkButton runat="server" class="btn btn-secondary" CommandArgument='<%#Eval("GRP_ID") + "+" + Eval("AC_GRP_NAME")%>' CommandName='<%#IPC.ACTIONPAGE.DELETE%>' OnClientClick="return ConfirmDelete('Are you sure you want to delete this entry?');">Delete</asp:LinkButton>
                                </td>
