...
namespace Utility
{
    public class ProcessLog
    {
        ...
        private static ILog CreateFileAppender(string appenderName, bool AddRoot = false, string datePattern = "-yyyy-MM-dd")
        {
            try
            {
                ...
                int maxSizeRollBackups = 25;

                string maxFileSize = "50MB";

                try
                {
                    maxSizeRollBackups = int.Parse(ConfigurationManager.AppSettings.Get("LogMaxSizeRollBackups"));
                }
                catch
                {
                    maxSizeRollBackups = 25;
                }

                try
                {
                    maxFileSize = ConfigurationManager.AppSettings.Get("LogMaxFileSize");
                }
                catch
                {
                    maxFileSize = "50MB";
